...
final class TargetAssetSvc: TargetMaking {
    ...
    init(sceneName: String = "Mango+Tree", treeH: Float = TargetCfg.Tree.height) {
        self.sceneName = sceneName
        sceneSvc = TargetSceneSvc(treeH: treeH)
    }

    func prepare() async -> Bool {
        if sceneTpl != nil { return true }
        if let task { return await finish(task) }
    ...
    func make() -> TargetPart? {
        guard let sceneTpl else { return fail("Target assets are not ready") }

        let real = sceneTpl.clone(recursive: true)
        real.name = "targetReal"
        sceneSvc.freeze(real)

        guard let size = sceneSvc.place(real) else { return fail("Scene bounds are empty") }
        guard mangoSvc.place(in: real) else { return fail("Could not place mango at a spawn point") }
        guard mangoSvc.mark(in: real) else { return fail("Mango entity not found in scene") }
        ...
        guard let parts = partSvc.make(pulse: pulse, trace: trace), !parts.isEmpty else {
        ...
        return TargetPart(root: root, real: real, parts: parts, height: size.y)
    }
