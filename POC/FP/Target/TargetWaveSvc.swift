...
struct TargetWaveSvc: TargetWaveChecking {
    func hitParts(
        ...
        let localStart = target.convert(
            position: data.start.pos,
            from: nil
        )
        ...
        for index in comp.parts.indices {
            ...
            let nearM = nearestDistance(
                from: localStart,
                to: part
            )
            
            guard nearM <= data.maxDistance else {
                continue
            }
            
            let center = target.convert(
