...
struct TargetSysTests {
    ...
    func dueEchoReveals() throws {
        let target = TestTarget.make()
        try TestTarget.setComp(target.entity) { $0.pendingRevealAt = [0: 500] }

        run(target.entity, now: 600)

        let comp = try TestTarget.comp(target.entity)
    ...
    func earlyEchoWaits() throws {
        let target = TestTarget.make()
        try TestTarget.setComp(target.entity) { $0.pendingRevealAt = [0: 900] }
    ...
    func revealSchedulesTheFade() throws {
        let target = TestTarget.make()
        try TestTarget.setComp(target.entity) { $0.pendingRevealAt = [0: 500] }

        run(target.entity, now: 600)

        let until = try TestTarget.comp(target.entity).pulseUntil[0]

        #expect(until == 600 + TargetCfg.Echo.pulseHoldS)
    }

    @Test("A pulse past its hold fades, leaving the trace behind")
    func pulseFades() throws {
        let target = TestTarget.make()
        target.parts[0].pulse.isEnabled = true
        target.parts[0].trace.isEnabled = true
        try TestTarget.setComp(target.entity) { $0.pulseUntil = [0: 500] }

        run(target.entity, now: 600)

        #expect(!target.parts[0].pulse.isEnabled)
        #expect(target.parts[0].trace.isEnabled)
        #expect(try TestTarget.comp(target.entity).pulseUntil.isEmpty)
    }

    @Test("An echo for a part that no longer exists is dropped")
    func staleIndexIsDropped() throws {
        let target = TestTarget.make(partCount: 1)
        try TestTarget.setComp(target.entity) { $0.pendingRevealAt = [7: 500] }
    ...
    func closeAndSeenIsFound() throws {
    ...
    func heardButFarIsNotFound() throws {
    ...
    func findingClearsEchoes() throws {
        let target = TestTarget.make(at: SIMD3<Float>(0, 0, -0.5))
        target.parts[0].pulse.isEnabled = true
    ...
    func firstBuzzMarksFeltAndSpeaks() throws {
    ...
    func tinyStepsDoNotBuzz() throws {
        let target = TestTarget.make(at: SIMD3<Float>(0, 0, -3))
        ...
        #expect(try !TestTarget.comp(target.entity).felt)
    }

    @Test("Turning to face a tree within reach buzzes on its own")
    func facingATreeBuzzes() throws {
    ...
    func facingADistantTreeIsQuiet() throws {
    ...
    func teachingSilencesGuidance() throws {
        let spy = NotificationSpy(.targetCue)
        let target = TestTarget.make(at: SIMD3<Float>(0, 0, -1.2))
        try TestTarget.setComp(target.entity) { $0.seenParts = [0] }
    ...
    func teachingDoesNotBlockTheReveal() throws {
        let target = TestTarget.make()
        try TestTarget.setComp(target.entity) { $0.pendingRevealAt = [0: 500] }

        run(target.entity, teaching: true, now: 600)

        #expect(try TestTarget.comp(target.entity).seenParts.contains(0))
    }
    ...
    func disabledIsSkipped() throws {
        ...
        #expect(try TestTarget.comp(target.entity).seenParts.isEmpty)
    }
}
