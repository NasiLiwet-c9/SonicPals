...
final class FPMeshRead: FPMeshReadPort {
    func read(
        ...
        var tris: [FPTri] = []
        tris.reserveCapacity(min(limit, 12_000))
        ...
        for anchor in anchors {
            ...
            for faceIndex in 0..<geo.faces.count {
                let face = geo.face(at: faceIndex)
                ...
                let tri = FPTri(
                    a: worldPos(
                        geo.vertex(at: face[0]),
                        transform: anchor.transform
                    ),
                    b: worldPos(
                        geo.vertex(at: face[1]),
                        transform: anchor.transform
                    ),
                    c: worldPos(
                        geo.vertex(at: face[2]),
                        transform: anchor.transform
                    )
                )
    ...
    private func evenlyLimited(
        ...
        let step = Double(tris.count) / Double(limit)
